# データ化
    ...
    execute store result score $x1 ui_temp run data get storage ui:common temp[0]
    execute store result score $y1 ui_temp run data get storage ui:common temp[1]
    execute store result score $z1 ui_temp run data get storage ui:common temp[2]

    summon marker ~ ~ ~ {Tags:["ui_common_marker"]}
    data modify storage ui:common temp set from entity @e[tag=ui_common_marker,limit=1] Pos
    execute store result score $x2 ui_temp run data get storage ui:common temp[0]
    execute store result score $y2 ui_temp run data get storage ui:common temp[1]
    execute store result score $z2 ui_temp run data get storage ui:common temp[2]
