...
execute at @s if score $success ui_temp matches 1 if block ^ ^ ^1.2 #ui:nocol run scoreboard players set $success ui_temp 0
execute at @s if score $success ui_temp matches 1 if block ^ ^ ^1.2 #ui:nocol run teleport @s ^ ^ ^1.2 ~ ~
#tellraw @a [{"text":"success: "},{"score":{"name": "$success","objective": "ui_temp"}}]
execute at @s if score $is ui_temp matches 20 run teleport @s ~ ~ ~ facing entity @e[distance=..60,predicate=ui:load_unhurtable,tag=!ui_temp_team,sort=random,limit=1]
...
execute store result storage ui:common input.Var2 int 1 run scoreboard players operation $temp ui_temp -= $is ui_temp
...
execute as @e[distance=..3,predicate=ui:load_unhurtable,tag=ui_temp_team] run function ui:common/particle/ss/downer/10016/effect
execute if score $is ui_temp matches 11.. if entity @e[distance=..2,predicate=ui:load_unhurtable,tag=!ui_temp_team] run function ui:common/particle/ss/downer/10017/detected
