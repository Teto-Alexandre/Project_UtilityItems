...
execute at @s if score $is ui_temp matches 20 run teleport @s ~ ~ ~ facing entity @e[distance=..50,predicate=ui:load_unhurtable,tag=!ui_temp_team,sort=random,limit=1]
execute at @s if score $success ui_temp matches ..0 if block ~ ~-0.1 ~ #ui:nocol run scoreboard players set $count ui_temp 0
execute at @s if score $success ui_temp matches ..0 if block ~ ~-0.1 ~ #ui:nocol run function ui:common/particle/ss/downer/10016/inair
...
execute if score $is ui_temp matches 11.. if entity @e[distance=..2,predicate=ui:load_unhurtable,tag=!ui_temp_team] run function ui:common/particle/ss/downer/10016/detected
