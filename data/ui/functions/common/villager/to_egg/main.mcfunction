...
summon item ~ ~ ~ {Item:{id:"minecraft:villager_spawn_egg",Count:1b,tag:{display:{Name:''},EntityTag:{CustomName:'',VillagerData:{level:2,type:"plains"},Invulnerable:1,Silent:1,NoAI:1,PersistenceRequired:1,Offers:{Recipes:[]}}}},Tags:["ui_common_villager"]}
data modify entity @e[tag=ui_common_villager,limit=1] Item.tag.EntityTag.Offers.Recipes set from storage ui:common input.Offers
data modify entity @e[tag=ui_common_villager,limit=1] Item.tag.EntityTag.CustomName set from block ~ ~ ~ CustomName
data modify entity @e[tag=ui_common_villager,limit=1] Item.tag.display.Name set from block ~ ~ ~ CustomName
tag @e[tag=ui_common_villager] remove ui_common_villager
