...
#見た目
    ...
    execute if score @s ui_bpart matches 104 run function ui:common/projectile/legacy/move/104
    execute if score @s ui_bpart matches 105 run function ui:common/projectile/legacy/move/105
...
#壁接触判定 - 液体の中ならスピードが最低値になる上に毎tick貫通力が減っていく
    execute positioned ^ ^ ^0.5 unless block ~ ~ ~ #ui:nocol run function ui:common/projectile/legacy/block_col
    execute as @s[tag=!ui_temp_col] positioned ^ ^ ^0.5 if block ~ ~ ~ #ui:liq run function ui:common/projectile/legacy/liq_col

#移動値が無くなるまで繰り返す
    execute as @s[scores={ui_temp=10..,ui_br=1..},tag=!ui_proj_common_nocol,tag=!ui_proj_common_hit,tag=!ui_temp_col] at @s run function ui:common/projectile/legacy/move