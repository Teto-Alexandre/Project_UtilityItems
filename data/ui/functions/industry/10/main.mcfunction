...
#レシピ
    ...
    #材料スロットのアイテム数をカウント
        execute store result score @s ui_i_supitem run data get block ~ ~ ~ Items[{Slot:10b}].Count
        execute store result score @s ui_i_supitem2 run data get block ~ ~ ~ Items[{Slot:11b}].Count
    #レシピを参照して必要素材と完成品をロード
        function ui:industry/10/recipe
...
#動作が進んだらアイテムを産出
    execute as @s[tag=ui_i_crafted] run function ui:industry/10/craft
...
#GUI描画
    execute if entity @s[scores={ui_i_gui=2}] run function ui:industry/10/gui
    
#土台壊されたら破壊
    execute as @s[tag=ui_i_break] run function ui:industry/10/break
