...
#電力をその座標から引き出す
    ...
    #蓄電機の給電速度を取得
        scoreboard players operation @s ui_i_suc = @s ui_i_sui
    #取得した給電速度<この座標にある電力
    #取得した給電速度<蓄電量差
        scoreboard players operation @s ui_i_suc < @e[tag=ui_i,distance=..0.5] ui_i_su
        scoreboard players operation @s ui_i_suc < $max ui_i_suc
    #給電速度の分だけ電力増加
        scoreboard players operation @s ui_i_su += @s ui_i_suc
    #この座標の電力を給電速度の分減少
        scoreboard players operation @e[tag=ui_i,distance=..0.5] ui_i_su -= @s ui_i_suc