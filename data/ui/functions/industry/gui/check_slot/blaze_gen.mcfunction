...
#各スロットにアイテムが入っているなら後ろのナンバーから順にセット
    ...
    execute if data block ~ ~ ~ Items[{Slot:10b}] unless data block ~ ~ ~ Items[{Slot:10b,tag:{ui:{ismenu:1}}}] run scoreboard players set $slot ui_calc1 10
    execute if data block ~ ~ ~ Items[{Slot:11b}] unless data block ~ ~ ~ Items[{Slot:11b,tag:{ui:{ismenu:1}}}] run scoreboard players set $slot ui_calc1 11
    ...
    execute if data block ~ ~ ~ Items[{Slot:13b,tag:{ui:{ismenu:1}}}] if score @s ui_i_item matches 1..63 run scoreboard players set @s ui_i_gui_mode 2
    execute if data block ~ ~ ~ Items[{Slot:13b,tag:{ui:{ismenu:1}}}] if score @s ui_i_item matches 64.. run scoreboard players set @s ui_i_gui_mode 3
    execute unless data block ~ ~ ~ Items[{Slot:13b}] run tag @s add ui_i_putout
...
#関係ないアイテムが入っているスロットからアイテムを排出
    execute if score $slot ui_calc1 matches 0..26 run function ui:industry/gui/check_slot/give_back/blaze_gen
