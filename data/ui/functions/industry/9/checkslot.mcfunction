...
#各スロットにアイテムが入っているなら後ろのナンバーから順にセット
    ...
    execute unless data block ~ ~ ~ Items[{Slot:26b}] run scoreboard players set @s[scores={ui_i_gui_recipe=6..}] ui_i_gui_recipe 1
    execute unless data block ~ ~ ~ Items[{Slot:26b}] run playsound block.dispenser.fail block @a ~ ~ ~ 0.8 1 0

#関係ないアイテムが入っているスロットからアイテムを排出
    execute if score $slot ui_calc1 matches 0..26 run function ui:industry/9/giveback
