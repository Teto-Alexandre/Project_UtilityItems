# 経験値が0以上ならレベルを上げる、上げたことと最終レベルを記録
    execute if score @s ui_lvl_craft_exp matches 0.. store success score $temp ui_temp store result score $level ui_temp run scoreboard players add @s ui_lvl_craft_lvl 1
...
# 10の倍数のレベルになったことを検知してカウントアップ
    ...
    execute if score @s ui_lvl_craft_exp matches 0.. run scoreboard players operation $temp2 ui_temp %= #10 ui_num
    execute if score @s ui_lvl_craft_exp matches 0.. if score $temp2 ui_temp matches 0 run scoreboard players add $temp3 ui_temp 1

# 経験値から次の必要経験値を減算する
    execute if score @s ui_lvl_craft_exp matches 0.. run scoreboard players operation @s ui_lvl_craft_exp -= $temp1 ui_temp
