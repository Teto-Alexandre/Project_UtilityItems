# 経験値が0以上ならレベルを上げる、上げたことと最終レベルを記録
    execute store result score $level ui_temp run scoreboard players add @s ui_lvl_cut_lvl 1
...
# 現在のレベルから次の必要経験値を計算する [ (lv*1.5 + 10)*lv + 100 ]q
    ...
    execute if score @s ui_lvl_cut_lvl matches 1000 run scoreboard players set $temp1 ui_temp 2147483647
...
# 経験値から次の必要経験値を減算する
    scoreboard players operation @s ui_lvl_cut_exp -= $temp1 ui_temp
