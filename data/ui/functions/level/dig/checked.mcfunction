...
#
    scoreboard players operation $level ui_temp = @a[tag=ui_temp_this] ui_lvl_dig_lvl
    scoreboard players operation $fortune ui_temp = @a[tag=ui_temp_this] ui_dig_f
...
#
    ...
    execute if score $temp ui_temp matches 1.. run function ui:level/dig/fortune

#
    execute if predicate ui:percentage/10 at @s run function ui:level/dig/level/additional

#
    scoreboard players operation @a[tag=ui_temp_this] ui_lvl_dig_exp += $xp ui_temp
