...
#
    scoreboard players operation $level ui_temp = @a[tag=ui_temp_this] ui_lvl_harv_lvl
    scoreboard players operation $fortune ui_temp = @a[tag=ui_temp_this] ui_harv_f
...
#
    ...
    execute if score $temp ui_temp matches 1.. run function ui:level/harv/fortune

#
    execute if predicate ui:percentage/10 at @s run function ui:level/harv/level/additional

#
    scoreboard players operation @a[tag=ui_temp_this] ui_lvl_harv_exp += $xp ui_temp
