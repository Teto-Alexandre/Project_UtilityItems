...
# レベルアップメッセージを送る
    ...
    execute if score $temp ui_temp matches 1 run tellraw @s [{"text":"  LEVEL UP! ","color":"aqua","bold":true},{"text":":","color":"gray"},{"text":" HARVESTING ","color":"yellow"},{"score":{"name":"$level.old","objective":"ui_temp"},"color":"gray"},{"text":" → ","color":"yellow","bold":true},{"score":{"name":"$level","objective":"ui_temp"},"color":"yellow"}]
    ...
    execute if score $temp ui_temp matches 1 run tellraw @s [{"text":"    Harvesting Fortune +","color":"yellow","bold":true},{"score":{"name":"$temp1","objective":"ui_temp"},"color":"yellow"}]
