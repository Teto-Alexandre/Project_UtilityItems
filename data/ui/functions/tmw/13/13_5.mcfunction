...
execute at @e[type=!#ui:notmob,sort=nearest,limit=1] positioned ~ ~1 ~ run teleport @s ~ ~ ~
scoreboard players add @s ui_tc 1

execute at @a[scores={ui_idc=0}] facing entity @s feet run teleport @a[scores={ui_idc=0}] ^ ^ ^0.8
effect give @a[scores={ui_idc=0}] slow_falling 1 0 true
...
execute at @a[scores={ui_idc=0}] if entity @s[distance=..1] run tag @a[scores={ui_idc=0}] add tmw_10_p2
