...
# ダメージ
    execute positioned ~-0.5 ~0.1 ~-0.5 positioned ~0.3 ~0.0 ~0.3 if entity @e[dx=0,tag=!ui_temp_team,type=!#ui:unhurtable,tag=!ui_unhurtable] at @s positioned ~-0.5 ~-0.4 ~-0.5 positioned ~-0.3 ~-0.3 ~-0.3 if entity @e[dx=0,tag=!ui_temp_team,type=!#ui:unhurtable,tag=!ui_unhurtable] at @s run function ui:tmw/235/proj/hit
...
# 接触判定: 非透過か液体と同じ座標なら破壊
    execute unless block ~ ~ ~ #ui:nocol run function ui:tmw/235/proj/kill
