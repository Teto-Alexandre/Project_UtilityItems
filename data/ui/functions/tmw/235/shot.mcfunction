...
# 弾を出す
    ...
    execute store result score $mod ui_temp run data get entity @s SelectedItem.tag.tmw.gun.ua 1
    #snipe係数をもとに範囲を絞る
    ...
    execute store result score @e[tag=ui_temp_unpower] ui_bm run data get entity @s SelectedItem.tag.tmw.gun.mov
    execute store result score @e[tag=ui_temp_unpower] ui_br run data get entity @s SelectedItem.tag.tmw.gun.range
    execute store result score @e[tag=ui_temp_unpower] ui_dmg run data get entity @s SelectedItem.tag.tmw.gun.dmg
    execute store result score @e[tag=ui_temp_unpower] ui_kb run data get entity @s SelectedItem.tag.tmw.gun.kb
...
# 発射音
    execute store result score $temp ui_temp run data get entity @s SelectedItem.tag.tmw.gun.snd
...
# クールタイム解除時刻.mod
    execute unless score $burst ui_temp matches 1 store result score $temp ui_temp run data get entity @s SelectedItem.tag.tmw.gun.ct1
    execute if score $burst ui_temp matches 1 store result score $temp ui_temp run data get entity @s SelectedItem.tag.tmw.gun.ct2
