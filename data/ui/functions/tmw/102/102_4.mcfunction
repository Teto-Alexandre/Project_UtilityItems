...
execute at @s[scores={ui_tc=20..69}] run scoreboard players operation @e[predicate=ui:load_unhurtable,distance=..2] ui_tmw102_id = @s ui_tmw102_id
execute as @e[predicate=ui:load_unhurtable] at @e[tag=ui_temp_this,limit=1] if score @s ui_tmw102_id = @e[tag=ui_temp_this,limit=1] ui_tmw102_id run teleport @s ~ ~ ~
...
execute at @s[scores={ui_tc=50}] as @e[predicate=ui:load_unhurtable,distance=..1] at @s run function ui:tmw/102/102_5
