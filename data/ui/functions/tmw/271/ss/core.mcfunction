# これ
    ...
    execute as @e[tag=tmw_271] if score $obj_id ui_temp = @s ui_obj_id run tag @s add ui_temp_obj
    scoreboard players operation $TurretType ui_temp = @s ui_is

#
    execute if score $TurretType ui_temp matches 1 run function ui:tmw/271/ss/id/1/
