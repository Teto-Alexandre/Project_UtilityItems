...
execute if entity @s[tag=ui_refd,tag=!tmw_256_noref] run scoreboard players set $deathmessage ui_temp 2
execute if entity @s[tag=tmw_256_deathmessage_14] run scoreboard players set $deathmessage ui_temp 1

#射程うんぬん
scoreboard players operation @s ui_temp = @s ui_bm
scoreboard players operation $speed ui_temp = @s ui_bm
...
#tick毎ループ
execute at @s[scores={ui_temp=10..}] run function ui:tmw/256/projectile/move
scoreboard players operation @s ui_bm_temp = @s ui_temp
tag @s[tag=ui_temp_col] remove ui_temp_col

#キル条件
execute at @s if score @s ui_br_temp >= @s ui_br run function ui:tmw/256/projectile/end
