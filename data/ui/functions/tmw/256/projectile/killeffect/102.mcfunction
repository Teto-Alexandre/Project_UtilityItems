...
execute as @e[distance=1..75,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/256/projectile/killeffect/102.2
...
execute as @e[distance=1..75,tag=!ui_temp_team,predicate=ui:load_unhurtable] if score @s ui_obj_id = $obj_id ui_temp at @s positioned ~ ~ ~ run function ui:tmw/256/attack/missile/fire_here
execute if score $success ui_temp matches 0 at @e[distance=..10,predicate=ui:load_unhurtable,sort=nearest,limit=1] positioned ~ ~ ~ run function ui:tmw/256/attack/missile/fire_here
