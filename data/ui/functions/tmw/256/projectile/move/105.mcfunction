...
# 当たり判定拡張
    execute at @e[tag=ui_temp_marker] positioned ~-0.5 ~-0.15 ~-0.5 as @e[dx=0,dy=0,dz=0,tag=!ui_temp_team,predicate=ui:load_unhurtable] at @s as @e[tag=ui_temp_this] run function ui:tmw/256/projectile/hit
# 見た目
    execute if score $team ui_temp matches 1 at @e[tag=ui_temp_marker] run function ui:tmw/256/projectile/move/105/1
    execute if score $team ui_temp matches 2 at @e[tag=ui_temp_marker] run function ui:tmw/256/projectile/move/105/2
    execute if score $team ui_temp matches 3 at @e[tag=ui_temp_marker] run function ui:tmw/256/projectile/move/105/3
    execute if score $team ui_temp matches 4 at @e[tag=ui_temp_marker] run function ui:tmw/256/projectile/move/105/4
# 時間カウント
    scoreboard players add @s ui_is 1
# 壁接触時
    execute at @e[tag=ui_temp_marker] unless block ~ ~ ~ #ui:nocol run tag @s add tmw_256_proj_del
    execute at @e[tag=ui_temp_marker] if block ~ ~ ~ #ui:liq run tag @s add tmw_256_proj_del
# 消す
