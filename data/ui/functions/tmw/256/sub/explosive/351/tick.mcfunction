...
# 常時実行
    ...
    execute if score @s ui_uses matches 4 run summon armor_stand ~ ~ ~ {Tags:["ui_temp_this","tmw_256_sub_351_1","ui_temp_obj"],Marker:1b,NoGravity:1b,Invisible:1b,ArmorItems:[{},{},{},{id:"minecraft:snowball",Count:1b,tag:{CustomModelData:217131}}]}
    ...
    execute if score @s ui_is matches 1..40 positioned ~ ~1 ~ run function ui:tmw/256/sub/explosive/351/tick.line
    ...
    execute if score @s ui_is2 matches 1.. positioned ~ ~1 ~ run function ui:tmw/256/sub/explosive/351/rain
    execute if score @s ui_is2 matches 1.. run schedule function ui:tmw/256/ss/1 1t append
    execute if score @s ui_is2 matches 1.. run kill @e[tag=ui_temp_particle]
    execute if score @s ui_is2 matches 80.. run tag @s add ui_256_sub_explode

# 処理終了
    execute if score @s ui_uses matches 4.. run tag @s add ui_256_sub_stop
    execute as @s[tag=ui_256_sub_stop] run scoreboard players add @s ui_is 1
    execute as @s[tag=ui_256_sub_stop,scores={ui_is=1..10}] run data merge entity @s {NoGravity:1b,Motion:[0.0d,0.0d,0.0d]}
    execute as @s[tag=ui_256_sub_stop2] run scoreboard players add @s ui_is2 1
    execute as @s[tag=ui_256_sub_stop2,scores={ui_is2=1}] run data merge entity @s {NoGravity:1b,Motion:[0.0d,0.0d,0.0d]}

# 処理終了（消える）
    execute if entity @s[tag=ui_256_sub_explode] run function ui:tmw/256/sub/explosive/351/end
