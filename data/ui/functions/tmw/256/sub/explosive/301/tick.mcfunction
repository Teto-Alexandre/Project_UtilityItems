...
# 常時実行
    execute if score @s ui_uses matches 5..14 run function ui:tmw/256/sub/explosive/301/setpos
    execute if score @s ui_is matches 1 run playsound block.tripwire.attach player @a ~ ~ ~ 2 0.8 0
    execute if score @s ui_is matches 1200.. run tag @s add ui_256_sub_stop
    execute positioned ~ ~-2 ~ if entity @e[distance=..4,tag=!ui_temp_team,predicate=ui:load_unhurtable] run tag @s add ui_256_sub_stop
    ...
    execute if score @s ui_is2 matches 20.. run tag @s add ui_256_sub_explode
    execute if score @s ui_is matches 20.. if score $team ui_temp matches 1 unless block ~ ~-3 ~ light_blue_wool run tag @s add ui_256_sub_explode
    execute if score @s ui_is matches 20.. if score $team ui_temp matches 2 unless block ~ ~-3 ~ pink_wool run tag @s add ui_256_sub_explode
    execute if score @s ui_is matches 20.. if score $team ui_temp matches 3 unless block ~ ~-3 ~ yellow_wool run tag @s add ui_256_sub_explode
    execute if score @s ui_is matches 20.. if score $team ui_temp matches 4 unless block ~ ~-3 ~ lime_wool run tag @s add ui_256_sub_explode
    #設置数上限デストロイ
    ...
    execute if score $count ui_temp matches 4.. run function ui:tmw/256/sub/explosive/mult.ex
...
# 処理終了
    ...
    execute if entity @s[tag=ui_256_sub_stop] run scoreboard players add @s ui_is2 1

# 処理終了（消える）
    execute if entity @s[tag=ui_256_sub_explode] positioned ~ ~-2 ~ run function ui:tmw/256/sub/explosive/301/end