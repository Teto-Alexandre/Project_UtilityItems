...
# ダメージを与える
    ...
    execute at @e[distance=4..5.5,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/256/projectile/hit.marker
    scoreboard players set $damage ui_temp 480
    execute at @e[distance=..4,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/256/projectile/hit.marker
