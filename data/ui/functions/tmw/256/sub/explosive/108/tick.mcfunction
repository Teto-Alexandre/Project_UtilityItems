...
# 常時実行
    ...
    execute if score @s ui_is matches 40.. run tag @s add ui_256_sub_explode

# 処理終了
    execute unless block ~ ~-0.1 ~ #ui:nocol run tag @s add ui_256_sub_stop
    execute unless block ~0.3 ~ ~ #ui:nocol run tag @s add ui_256_sub_stop
    execute unless block ~-0.3 ~ ~ #ui:nocol run tag @s add ui_256_sub_stop
    execute unless block ~ ~ ~0.3 #ui:nocol run tag @s add ui_256_sub_stop
    execute unless block ~ ~ ~-0.3 #ui:nocol run tag @s add ui_256_sub_stop
    execute unless block ~ ~0.5 ~ #ui:nocol run tag @s add ui_256_sub_stop
    ...
    execute as @s[tag=ui_256_sub_stop,scores={ui_is=1}] run data merge entity @s {NoGravity:1b,Motion:[0.0d,0.0001d,0.0d]}

# 処理終了
    execute if entity @s[tag=ui_256_sub_explode] run function ui:tmw/256/sub/explosive/108/end