summon item ~ ~ ~ {Tags:["ui_temp_this","tmw_256_sub"],Invulnerable:1b,Item:{id:"minecraft:snowball",Count:1b,tag:{tmw:{type:1,color:1},CustomModelData:217111}},PickupDelay:32767s}
...
execute if score $activator ui_temp matches 101 run function ui:tmw/256/sub/model/101
execute if score $activator ui_temp matches 103 run function ui:tmw/256/sub/model/103
...
execute if score $sptype ui_temp matches 253 if score $sptime ui_temp matches 1.. run tag @e[tag=ui_temp_this,limit=1] add tmw256_sub_nopoint
