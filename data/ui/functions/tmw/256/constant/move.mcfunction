...
# チャージキープ
    execute if score $ishold ui_temp matches 2.. run function ui:tmw/256/constant/charge_cancel
...
# 特殊効果
    ...
    execute as @e[nbt={HurtTime:0s}] if score @s ui_st2 matches 20.. run effect give @s instant_health 1 2 true
...
# 移動形態になるとダブルジャンプが復活
    tag @s[tag=tmw_256_jumped] remove tmw_256_jumped
