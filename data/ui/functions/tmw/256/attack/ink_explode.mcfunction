...
# ダメージを与える
    ...
    execute at @e[distance=2.5..5,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/256/projectile/hit.undeath
    scoreboard players set $damage ui_temp 150
    execute at @e[distance=..2.5,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/256/projectile/hit.undeath
