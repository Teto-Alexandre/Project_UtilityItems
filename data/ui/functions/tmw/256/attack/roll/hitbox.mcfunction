# 当たり判定発生
    ...
    execute at @e[distance=..1.2,tag=!ui_temp_team,predicate=ui:load_unhurtable,tag=!ui_temp_attacked] run function ui:tmw/256/attack/roll/hit
...
# 右側へ移動.lp
    ...
    execute if score $width ui_temp matches 1.. positioned ^ ^ ^0.8 run function ui:tmw/256/attack/roll/hitbox