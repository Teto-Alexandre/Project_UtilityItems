...
# チャージ補正
    execute if score $burst_alt.id ui_temp matches 1..100 run function ui:tmw/256/attack/shot/burst4
    execute if score $burst_alt.id ui_temp matches 101..200 run function ui:tmw/256/attack/shot/burst5
    execute if score $burst_alt.id ui_temp matches 201..300 run function ui:tmw/256/attack/shot/burst8
    execute if score $burst_alt.id ui_temp matches 301..400 run function ui:tmw/256/attack/shot/burst9

# タイプごとに拡散して発射
    #function ui:tmw/256/attack/shot/loop
    execute if score $spreadtype ui_temp matches 1 anchored eyes positioned ^ ^ ^ run function ui:tmw/256/attack/shot/spreadmanager/1
    execute if score $spreadtype ui_temp matches 2 anchored eyes positioned ^ ^ ^ run function ui:tmw/256/attack/shot/spreadmanager/2
    execute if score $spreadtype ui_temp matches 3 anchored eyes positioned ^ ^ ^ run function ui:tmw/256/attack/shot/spreadmanager/3

# スコア依存で足元に塗り判定発生
    execute store result score $temp ui_temp run data get storage ui:gun temp2.ShotFootStep
    execute if score $temp ui_temp matches 1.. run function ui:tmw/256/attack/shot/shotfootstep/manager
