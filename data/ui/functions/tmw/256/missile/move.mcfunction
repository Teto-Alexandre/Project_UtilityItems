...
# マーカーに実行
    execute at @e[tag=ui_temp_marker] run function ui:tmw/256/missile/move.marker

# 移動値が無くなるまで繰り返す
    execute as @s[scores={ui_temp=10..,ui_br=1..},tag=!ui_proj_common_nocol,tag=!ui_proj_common_hit,tag=!ui_temp_col] at @s run function ui:tmw/256/missile/move