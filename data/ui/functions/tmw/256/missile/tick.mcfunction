...
#tick毎ループ
execute at @s[scores={ui_temp=10..}] run function ui:tmw/256/missile/move
scoreboard players operation @s ui_bm_temp = @s ui_temp
tag @s[tag=ui_temp_col] remove ui_temp_col

#キル条件
execute as @s[scores={ui_br=..0}] run function ui:tmw/256/projectile/kill
