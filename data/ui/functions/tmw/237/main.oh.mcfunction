...
execute if score $temp ui_temp matches 1.. run summon area_effect_cloud ~ ~ ~ {Duration:6000,Age:4,Effects:[{Id:2b,Amplifier:0b,Duration:6,ShowParticles:0b}],Tags:["ui","tmw_237.snipe"]}
...
execute if score $temp ui_temp matches 1.. store result entity @e[tag=tmw_237.snipe,sort=nearest,limit=1] Effects.[0].Amplifier byte 1 run scoreboard players get $temp ui_temp
