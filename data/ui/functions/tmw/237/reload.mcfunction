# $ink = $ink.max になるまで.lp
    execute if score $cooltime ui_temp matches 0 if entity @s[nbt={ActiveEffects:[{Id:15}]}] run scoreboard players operation $ink.s ui_temp /= #2 ui_num
    execute if score $cooltime ui_temp matches 0 if entity @s[nbt={ActiveEffects:[{Id:15}]}] run scoreboard players operation $ink.m ui_temp /= #2 ui_num
