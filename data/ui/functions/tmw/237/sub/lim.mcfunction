...
# サブウェポン２発動
execute if score $qf ui_temp matches 1..2 if score $ink ui_temp < $ink.sp ui_temp run function ui:tmw/237/sub/fail
execute if score $qf ui_temp matches 1..2 if score $ink ui_temp >= $ink.sp ui_temp at @s anchored eyes positioned ^ ^ ^ run function ui:tmw/237/sub/core
