...
# 処理終了
    ...
    #execute as @s[tag=ui_237_sub_stop] if score $time ui_temp matches 0 run scoreboard players remove @s ui_is 1
    execute as @s[tag=ui_237_sub_stop,tag=!ui_237_sub_first] run data merge entity @s {NoGravity:1b,Motion:[0.0d,0.0001d,0.0d]}
    execute as @s[tag=ui_237_sub_stop,tag=!ui_237_sub_first] run summon cow ~ ~ ~ {Tags:["ui_temp_this","tmw_237_sub_111_1","tmw_237_sub_mult","ui_temp_obj","tds_nolog","ui_temp_team"],NoAI:1b,ActiveEffects:[{Id:14b,Amplifier:0,Duration:20000,ShowParticles:false}],DeathLootTable:"none",Attributes:[{Name:"generic.max_health",Base:1000}],Health:1000f,Silent:1}
    ...
    execute if score $count ui_temp matches 2.. run function ui:tmw/237/sub/explosive/mult
    tag @e[tag=ui_temp_this] remove ui_temp_this
    #ダメージ計算
    execute if entity @s[tag=ui_237_sub_first] store result score $temp ui_temp run data get entity @e[tag=ui_temp_obj,tag=tmw_237_sub_111_1,limit=1] Health 10
    ...
    execute if entity @s[tag=ui_237_sub_first] run data merge entity @e[tag=ui_temp_obj,tag=tmw_237_sub_111_1,limit=1] {Health:1000f}

# 射撃
    execute if entity @s[tag=ui_237_sub_stop] run teleport @s ~ ~ ~ ~3 ~
    execute if entity @s[tag=ui_237_sub_stop] if score $time ui_temp matches 0 rotated ~ -60 run function ui:tmw/237/sub/explosive/111/rain
    execute if entity @s[tag=ui_237_sub_stop] if score $time ui_temp matches 2 rotated ~180 -60 run function ui:tmw/237/sub/explosive/111/rain

# 処理終了（消える）
    execute if entity @s[tag=ui_237_sub_explode] run function ui:tmw/237/sub/explosive/111/end
