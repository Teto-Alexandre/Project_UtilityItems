...
# 同じidのビットを召喚
    #外側
    execute if score @s ui_uses matches 20 run summon armor_stand ~ ~ ~ {Tags:["ui_temp_this","tmw_237_sub_153_1","ui_temp_obj"],Invisible:1b,NoGravity:1b,Marker:1b}
    #idコピー
...
# ラインを配置して座標を塗り回転する（これを4回繰り返す）
    ...
    execute as @e[tag=tmw_237_sub_153_1,tag=ui_temp_obj] at @s if score $age ui_temp matches 1.. run function ui:tmw/237/sub/explosive/153/tick.rotator
    execute as @e[tag=tmw_237_sub_153_1,tag=ui_temp_obj] at @s run teleport @s ~ ~ ~ ~24 ~
...
# 当たり判定
    execute if score $temp2 ui_temp matches 0 if score @s ui_uses matches 21..24 at @e[distance=..2,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/sub/explosive/153/hit
    execute if score $temp2 ui_temp matches 0 if score @s ui_uses matches 25..28 at @e[distance=..3,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/sub/explosive/153/hit
    execute if score $temp2 ui_temp matches 0 if score @s ui_uses matches 29..32 at @e[distance=..4,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/sub/explosive/153/hit
    execute if score $temp2 ui_temp matches 0 if score @s ui_uses matches 33..36 at @e[distance=..5,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/sub/explosive/153/hit
    execute if score $temp2 ui_temp matches 0 if score @s ui_uses matches 37..40 at @e[distance=..6,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/sub/explosive/153/hit
