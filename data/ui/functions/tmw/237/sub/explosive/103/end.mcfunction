...
# ダメージを与える
    execute if score $color ui_temp matches 1 run data modify storage ui:temp Name set value '{"text":"クイックボム","color":"aqua"}'
    execute if score $color ui_temp matches 2 run data modify storage ui:temp Name set value '{"text":"クイックボム","color":"light_purple"}'
