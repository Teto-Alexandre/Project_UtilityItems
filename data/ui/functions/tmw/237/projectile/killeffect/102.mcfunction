...
execute as @e[distance=1..100,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/projectile/killeffect/102.2
...
execute as @e[distance=1..100,tag=!ui_temp_team,predicate=ui:load_unhurtable] if score @s ui_obj_id = $obj_id ui_temp at @s positioned ~ ~ ~ run function ui:tmw/237/attack/missile/fire_here
