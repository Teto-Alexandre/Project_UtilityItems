...
# 弾を出す
    ...
    execute if score $spread ui_temp matches 1.. run function ui:tmw/237/attack/missile/spreadmanager/square_shuffle
    execute facing entity @e[tag=ui_marker,limit=1] feet run function ui:tmw/237/attack/missile/fire_straight
    kill @e[tag=ui_marker]

# ショットガン.lp
    scoreboard players remove $multishot ui_temp 1
    execute if score $multishot ui_temp matches 1.. run function ui:tmw/237/attack/missile/spreadmanager/1