...
# チャージ補正
    #execute if score $burst_alt.id ui_temp matches 1..100 run function ui:tmw/237/attack/missile/burst4
    #execute if score $burst_alt.id ui_temp matches 101..200 run function ui:tmw/237/attack/missile/burst5

# タイプごとに拡散して発射
    #function ui:tmw/237/attack/missile/loop
    execute anchored eyes positioned ^ ^ ^ rotated ~ -90 run function ui:tmw/237/attack/missile/spreadmanager/1

# スコア依存で足元に塗り判定発生
    execute store result score $temp ui_temp run data get storage ui:gun temp2.ShotFootStep
    execute if score $temp ui_temp matches 1.. run function ui:tmw/237/attack/missile/shotfootstep/manager
