...
# 再入力受付
    ...
    execute if entity @s[tag=tmw_active_temp] if score $firetime.temp ui_temp matches 8.. if score $bullets ui_temp matches 1.. in overworld store result score $firetime ui_temp run time query gametime
