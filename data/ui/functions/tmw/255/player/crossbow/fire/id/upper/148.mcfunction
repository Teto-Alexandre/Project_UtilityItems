...
#
    ...
    execute if score $firetime.temp ui_temp matches 0 run scoreboard players set $Model ui_temp 180083
    execute if score $firetime.temp ui_temp matches 10 run data merge storage ui:common {input:{Mode:"create",Var:-10034,Var2:3}}
    execute if score $firetime.temp ui_temp matches 10 at @s anchored eyes rotated ~ 0 positioned ^ ^ ^4 run function ui:common/particle
    execute if score $firetime.temp ui_temp matches 10 run scoreboard players set $success ui_temp 1
    execute if score $firetime.temp ui_temp matches 10 run scoreboard players set $Model ui_temp 180082
