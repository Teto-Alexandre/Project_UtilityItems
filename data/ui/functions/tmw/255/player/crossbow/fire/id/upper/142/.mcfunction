# ミミック: 核
    ...
    execute if entity @s[tag=tmw_active_temp] run function ui:tmw/255/player/crossbow/fire/id/upper/142/trigger

#
    scoreboard players set $firetime.max ui_temp 600
...
#
    ...
    #execute if score $firetime.temp ui_temp matches 0 run data merge storage ui:common {input:{Mode:"create",Var:-10032,Var2:2,Rand:{RotY:1}}}
    ...
    #execute if score $firetime.temp ui_temp matches 3 run data merge storage ui:common {input:{Mode:"create",Var:-10032,Var2:2,Rand:{RotY:1}}}
    ...
    #execute if score $firetime.temp ui_temp matches 10 run data merge storage ui:common {input:{Mode:"create",Var:-10032,Var2:2}}
...
#
    ...
    execute unless score $success ui_temp matches 1 if score $customcount ui_temp matches 1 run function ui:tmw/255/player/crossbow/fire/id/upper/142/1
    execute unless score $success ui_temp matches 1 if score $customcount ui_temp matches 2 run function ui:tmw/255/player/crossbow/fire/id/upper/142/2
    execute unless score $success ui_temp matches 1 if score $customcount ui_temp matches 3 run function ui:tmw/255/player/crossbow/fire/id/upper/142/3
    execute unless score $success ui_temp matches 1 if score $customcount ui_temp matches 11 run function ui:tmw/255/player/crossbow/fire/id/upper/142/11

#
    #execute if score $firetime ui_temp matches 0 run say 内部コンボ係数=1
    execute if score $firetime ui_temp matches 0 run scoreboard players set $customcount ui_temp 1