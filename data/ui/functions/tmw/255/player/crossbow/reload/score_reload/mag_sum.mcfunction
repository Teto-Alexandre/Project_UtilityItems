...
# 装弾数が最大装弾数より少ないなら弾を消費
    ...
    execute if score $magazine ui_temp matches 1.. run function ui:tmw/255/player/crossbow/reload/score_reload/lim
...
#
    execute if score $add ui_temp matches 1 run function ui:tmw/255/player/crossbow/reload/score_reload/mag_sum