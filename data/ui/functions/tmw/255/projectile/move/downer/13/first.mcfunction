...
execute as @a[distance=1..200,tag=!ui_temp_player,predicate=ui:load_unhurtable,sort=random,limit=1] run tag @s add ui_temp_target
execute unless entity @a[tag=ui_temp_target] as @e[distance=1..200,tag=!ui_temp_player,predicate=ui:load_unhurtable,sort=random,limit=1] run tag @s add ui_temp_target
