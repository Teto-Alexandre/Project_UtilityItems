...
execute as @a if score $id ui_temp = @s ui_id run tag @s[predicate=ui:is_sneaking] add ui_temp_player2
...
execute as @e[distance=1..200,tag=!ui_temp_team,predicate=ui:load_unhurtable] if score @s ui_obj_id = $obj_id ui_temp run tag @s add ui_temp_target
    ...
    execute unless entity @a[tag=ui_temp_player2] run function ui:tmw/255/projectile/move/downer/12/normal
    execute if entity @a[tag=ui_temp_player2] unless score $obj_id ui_temp matches 1.. run function ui:tmw/255/projectile/move/downer/12/normal

    execute if entity @a[tag=ui_temp_player2] if score $obj_id ui_temp matches 1.. run function ui:tmw/255/projectile/move/downer/12/teleport
