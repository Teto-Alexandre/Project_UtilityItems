...
# ターゲット出来るエンティティを周囲に表示
    execute as @e[distance=1..100,predicate=ui:load_unhurtable] unless score @s ui_id = $id ui_temp unless score @s ui_team = $team ui_temp run function ui:tmw/242/target_marker
...
# 発射ボタンを押せば青マーカーの相手に全てのミサイルが飛ぶ
    execute at @s[scores={ui_use1=1..}] run function ui:tmw/242/sweep_attack
