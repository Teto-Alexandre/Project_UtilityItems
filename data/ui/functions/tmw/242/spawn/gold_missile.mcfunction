# ミサイル召喚
    summon armor_stand ~ ~ ~ {Tags:["tag","tmw_242_f"],Motion:[0.0d,0.0d,0.0d],Invisible:1b,Invulnerable:1b,Small:1b}
    #召喚した本体に固有ID設定
    ...
    #タイプを選択
    scoreboard players set @e[tag=tag] ui_gms 4
    scoreboard players set @e[tag=tag] ui_gt 6
    scoreboard players set @e[tag=tag] ui_bt 3
    ...
    scoreboard players add $rand ui_calc1 40
