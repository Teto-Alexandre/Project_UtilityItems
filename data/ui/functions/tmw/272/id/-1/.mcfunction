...
execute if entity @s[tag=tmw_oh_s,scores={ui_st2=..20},gamemode=creative] run function ui:tmw/272/id/-1/clone
execute if entity @s[scores={ui_st2=41},gamemode=creative] at @s run particle dust 1 0 1 2 ~ ~ ~ 1 1 1 0 15 force
execute if entity @s[scores={ui_st2=41},gamemode=creative] at @s run playsound entity.enderman.teleport player @a ~ ~ ~ 1 0.5 0
execute if entity @s[scores={ui_st2=41},gamemode=creative] at @s run tellraw @s [{"text":"[Idling] > ","color": "light_purple"},{"text":"長Sneak+Fキーで羊を分解","color": "gray"}]
execute if entity @s[tag=tmw_oh_s,scores={ui_st2=41..},gamemode=creative] as @e[type=sheep,sort=nearest,limit=1] at @s run function ui:tmw/272/id/-1/sheep_decomposer
