...
data modify storage ui:temp temp.temp.effect set value {effect_type:"after_effect",cg:{cost:{amount:0,bypass:1},delay:1,effects:[]}}

data modify storage ui:temp temp.temp.effect.cg.effects set from storage ui:temp temp.effect
