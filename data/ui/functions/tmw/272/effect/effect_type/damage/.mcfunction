...
function ui:tmw/272/common/inport_text
execute as @e[tag=tmw272_temp_card_effect_target] if score @s ui_tmw272_invulnerable matches 1.. run tellraw @a[tag=ui_temp_players] ["",{"text":"   ","color":"gray"},{"selector":"@s"},{"text":"は"},{"storage":"ui:tmw272_text","nbt":"temp.name","interpret":true     ,"hoverEvent": {"action": "show_text","value":[{"storage":"ui:tmw272_text","nbt":"temp.hover","interpret":true}]}},{"text":"！"}]
execute as @e[tag=tmw272_temp_card_effect_target] if score @s ui_tmw272_invulnerable matches 1.. run tag @s remove tmw272_temp_card_effect_target
