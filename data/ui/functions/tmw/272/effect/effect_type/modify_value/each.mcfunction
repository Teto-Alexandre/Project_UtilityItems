...
execute if data storage ui:temp temp.effect{input:"health"} unless data storage ui:temp temp.effect{effect_mode:"set"} if score $var ui_temp matches ..-1 run scoreboard players add @s[tag=ui_temp_player] ui_tmw272_decay 1
execute if data storage ui:temp temp.effect{input:"health"} if data storage ui:temp temp.effect{effect_mode:"set"} if score $var ui_temp < @s ui_tmw272_health run scoreboard players add @s[tag=ui_temp_player] ui_tmw272_decay 1
