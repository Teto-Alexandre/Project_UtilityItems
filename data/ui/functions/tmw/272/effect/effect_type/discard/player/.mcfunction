...
execute unless data storage ui:temp temp.effect.all unless data storage ui:temp temp.effect.only_one store result score $condition_checker_temp_temp ui_temp run clear @s #ui:all{IsCG1:1} 1
execute unless data storage ui:temp temp.effect.all if data storage ui:temp temp.effect.only_one store result score $condition_checker_temp_temp ui_temp run clear @s #ui:all{IsCG1:2} 1
execute if data storage ui:temp temp.effect.all store result score $condition_checker_temp_temp ui_temp run clear @s #ui:all 1
