...
$execute unless data storage ui:temp temp.effect.only_one run data modify storage ui:temp temp.effect.temp1 append from entity @s Inventory[{tag:{IsCG1:1,$(macro)}}]
$execute if data storage ui:temp temp.effect.only_one run data modify storage ui:temp temp.effect.temp1 append from entity @s Inventory[{tag:{IsCG1:2,$(macro)}}]
...
$execute unless data storage ui:temp temp.effect.only_one store result score $condition_checker_temp_temp ui_temp run clear @s #ui:all{IsCG1:1,$(macro)} 1
$execute if data storage ui:temp temp.effect.only_one store result score $condition_checker_temp_temp ui_temp run clear @s #ui:all{IsCG1:2,$(macro)} 1
