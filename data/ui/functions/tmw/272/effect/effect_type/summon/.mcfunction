...
execute unless data storage ui:temp temp.effect.id run summon pig ~ ~ ~ {NoAI:1b,Tags:["hc_deck","summoned_nocount","summoned_delete","hc_deck_temp","ui_temp_players"],Invulnerable:1b,CustomNameVisible:1b,Silent:1b}
...
execute as @e[tag=hc_deck_temp,type=item_display] at @s run teleport @s ~ ~1.5 ~
execute as @e[tag=hc_deck_temp,type=block_display] at @s run teleport @s ~ ~1.5 ~
...
execute as @e[tag=hc_deck_temp] run tag @s add tmw272_temp_summoned
