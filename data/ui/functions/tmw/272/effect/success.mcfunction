...
# コスト変動
execute if score $broadcast_type ui_temp matches 1 if score @s ui_tmw272_cost_next matches 1.. run scoreboard players set @s ui_tmw272_cost_next 0
...
# 最終使用カードに登録
execute if score $broadcast_type ui_temp matches 1 if score $no_last_used ui_temp matches 0 run data modify storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].ui.cg1.LastUsed set from storage ui:temp card
