...
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_duplication run execute store result score $duplication ui_temp run scoreboard players get @s ui_tmw272_duplication
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_duplication run execute if score $duplication ui_temp matches 1.. run function ui:tmw/272/effect/misc/duplication/
...
# 出血チェック
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_bleed if score @s ui_tmw272_bleed matches 1.. run function ui:tmw/272/effect/misc/bleed
# 出血チェック
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_fairy if score @s ui_tmw272_fairy matches 1.. run function ui:tmw/272/effect/misc/fairy
# 帯電チェック
...
# コスト変動
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_cost_next unless score @s ui_tmw272_cost_next matches 0 run scoreboard players set @s ui_tmw272_cost_next 0
# 必中
execute if score $broadcast_type ui_temp matches 1 run scoreboard players set $surehit_checker ui_temp 0
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_surehit if score @s ui_tmw272_surehit matches 1.. run scoreboard players set $surehit_checker ui_temp 1
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_surehit if score @s ui_tmw272_surehit matches 1.. run data merge storage ui:tmw272 {temp:{input:"surehit"}}
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_surehit if score @s ui_tmw272_surehit matches 1.. run function ui:tmw/272/common/value/inport_text with storage ui:tmw272 temp
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_surehit if score @s ui_tmw272_surehit matches 1.. run tellraw @a[tag=ui_temp_players] ["",{"storage":"ui:tmw272_text","nbt":"temp.title","interpret":true     ,"hoverEvent": {"action": "show_text","value":[{"storage":"ui:tmw272_text","nbt":"temp.hover","interpret":true}]}},{"text":": ","color":"gray"},{"selector":"@s"},{"text":"の"},{"storage":"ui:tmw272_text","nbt":"temp.name","interpret":true     ,"hoverEvent": {"action": "show_text","value":[{"storage":"ui:tmw272_text","nbt":"temp.hover","interpret":true}]}},{"text":"は"},{"storage":"ui:tmw272_text","nbt":"temp.score","interpret":true},{"text": "！"}]
...
# 帯電
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_shock if score $shock_checker ui_temp >= @s ui_tmw272_shock if score @s ui_tmw272_shock matches 1.. run function ui:tmw/272/effect/misc/shock
...
# 最終使用カードに登録
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep if score $no_last_used ui_temp matches 0 run data modify storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].ui.cg1.LastUsed set from storage ui:temp card
...
#
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_chain run scoreboard players add @s ui_tmw272_chain 1
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp temp.card.indep unless data storage ui:temp temp.card.no_surehit if score $surehit_checker ui_temp matches 1 run scoreboard players remove @s ui_tmw272_surehit 1
