...
#
execute if score $turn_max ui_temp matches 1.. if score $turn_name ui_temp >= $turn_limit ui_temp run tellraw @s ["",{"text":"このカードは","color":"gray"},{"score":{"name": "$turn_max","objective": "ui_temp"},"color":"gray"},{"text":"ターンに","color":"gray"},{"score":{"name": "$turn_limit","objective": "ui_temp"},"color":"gray"},{"text":"回しか使用できません","color":"gray"}]
execute if score $turn_max ui_temp matches ..-1 if score $turn_name ui_temp >= $turn_limit ui_temp run tellraw @s ["",{"text":"このカードは戦線中ではもう使用できません","color":"gray"}]
