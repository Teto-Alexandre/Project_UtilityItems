...
# 条件から外れていればダメ
execute unless data storage ui:temp temp.mod.mute unless data storage ui:temp card.cost.target if score $cost_min ui_temp matches -2147483648..2147483647 if score $cost_act ui_temp < $cost_min ui_temp run tellraw @s [{"text":"    条件が満たされていません","color":"gray"}]
...
execute unless data storage ui:temp temp.mod.mute unless data storage ui:temp card.cost.target if score $cost_max ui_temp matches -2147483648..2147483647 if score $cost_act ui_temp > $cost_max ui_temp run tellraw @s [{"text":"    条件が満たされていません","color":"gray"}]
