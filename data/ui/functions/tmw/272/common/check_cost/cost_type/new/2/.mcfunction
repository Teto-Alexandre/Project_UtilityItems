...
execute if data storage ui:temp card.cost{target:1} if score $cost_count ui_temp matches 1.. as @e[tag=tmw272_visual_input] run function ui:tmw/272/common/check_cost/cost_type/new/2/burn
execute unless data storage ui:temp card.cost{target:1} if score $cost_count ui_temp matches 1.. run function ui:tmw/272/common/check_cost/cost_type/new/2/burn
