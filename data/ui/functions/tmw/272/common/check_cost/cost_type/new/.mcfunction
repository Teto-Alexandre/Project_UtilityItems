...
# ここで封印
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp card.no_seal run scoreboard players operation $cost ui_temp += @s ui_tmw272_seal
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp card.no_cost_next run scoreboard players operation $cost ui_temp += @s ui_tmw272_cost_next
...
# スタンしているなら問答無用で却下
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp card.no_stun if score @s ui_tmw272_stun matches 1.. unless score $bypass ui_temp matches 1.. run tellraw @s [{"text":"    行動不能状態です","color":"gray"}]
execute if score $broadcast_type ui_temp matches 1 unless data storage ui:temp card.no_stun if score @s ui_tmw272_stun matches 1.. unless score $bypass ui_temp matches 1.. run scoreboard players set $cost_type ui_temp -1
