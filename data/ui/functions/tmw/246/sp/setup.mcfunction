...
#コアの固有IDを設定
execute at @s run scoreboard players operation @s ui_id = @p ui_id
tellraw @a [{"text":"System>","color":"gray"},{"text":"id:","color":"green"},{"score":{"name":"@s","objective":"ui_id"},"color":"green"},{"text":"のダンジョンコアが発生しました","color":"green"}]
...
#コアの当たり判定を設定
execute at @s align xyz positioned ~0.5 ~0.5 ~0.5 run summon minecraft:iron_golem ~ ~2.5 ~ {Tags:["ui","ui_hitbox","ui_h_c","tds_nolog"],Silent:1b,NoAI:1b,NoGravity:1b,active_effects:[{id:"minecraft:invisibility",duration:20000,amplifier:1b,show_particles:0b}],Attributes:[{Name:"generic.max_health",Base:1000}],Health:1000.0f,DeathLootTable:"ui:empty"}
