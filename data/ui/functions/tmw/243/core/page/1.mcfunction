...
# スロットが空白ならボタンが押されたと判断する＋changedを1に
    ...
    execute if score $slot ui_temp matches 22 run scoreboard players set $sub ui_temp 201
    execute if score $slot ui_temp matches 23 run scoreboard players set $sub ui_temp 301
    execute if score $slot ui_temp matches 24 run scoreboard players set $sub ui_temp 401
