...
# 前提条件が一つでもかけていれば弾く
    ...
    #条件ごとに実行する
        ...
        execute if score $needtotal ui_temp matches 1.. run function ui:tmw/243/exo/needed_checker/list_master
        execute if score $mettotal ui_temp < $needtotal ui_temp run scoreboard players set $cancel ui_temp 1
        execute if score $mettotal ui_temp < $needtotal ui_temp run tellraw @a ["",{"text":"> 前提スキルセットが足りません "},{"score":{"name":"$mettotal","objective":"ui_temp"},"color":"red"},{"text":" < "},{"score":{"name":"$needtotal","objective":"ui_temp"},"color":"red"}]
    #条件リストを削除
...
# 既に同じ数値を持っていたら弾く
    ...
    #コアのスキルごとに照合
        function ui:tmw/243/exo/double_checker/liner_search
    #コアのスキルリストを削除
...
# 入力した必要ポイント未満なら弾く
    ...
    #足りないなら弾く
        ...
        execute if score $point ui_temp < $needpoint ui_temp run tellraw @a ["",{"text":"> スキルポイントが足りません "},{"score":{"name":"$point","objective":"ui_temp"},"color":"red"},{"text":" < "},{"score":{"name":"$needpoint","objective":"ui_temp"},"color":"red"}]
...
# 音鳴らして取得させる、実行者に返す
    ...
    #反映
        execute if score $cancel ui_temp matches 0 run item modify entity @s container.17 ui:tmw243/core
