...
# 同じ数字がコア内にあるか線形探索
    ...
    execute if score $core ui_temp = $id ui_temp run tellraw @a ["",{"text":"> 同じスキルを既に習得しています"}]

# 参照した条件を削除
    data remove storage ui:temp2 temp[0]

# 条件が残っているなら、さらに条件ごとに実行する
    execute if data storage ui:temp2 temp[0] run function ui:tmw/243/exo/double_checker/liner_search