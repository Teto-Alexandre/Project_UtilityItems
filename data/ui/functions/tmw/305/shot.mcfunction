...
#弾を出す
summon minecraft:armor_stand ~ ~ ~ {Marker:1b,NoGravity:1b,Invisible:1b,Tags:["tmw_proj","ui","ui_proj","ui_305_1","ui_305_unpower","ui_305_nohit"]}
...
execute store result score $mod ui_calc1 run data get entity @s SelectedItem.tag.tmw.gun.ua 1
...
execute as @e[tag=ui_305_unpower] at @s facing entity @e[tag=ui_marker,limit=1] feet positioned ~ ~1.6 ~ run teleport @s ^ ^ ^1 ~ ~
execute if entity @s[scores={ui_st=1..}] as @e[tag=ui_305_unpower] at @s run teleport @s ~ ~-0.3 ~ ~ ~
