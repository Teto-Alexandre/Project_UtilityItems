# ミサイルが飛んできていたら当たったことがわかる装備
    execute if entity @s[nbt={active_effects:[{id:"minecraft:glowing"}]}] run scoreboard players add @s tds_amethyst 10000
    execute if entity @s[nbt={active_effects:[{id:"minecraft:glowing"}]}] run scoreboard players set @s tds_amethyst_t 10
    execute if entity @s[nbt={active_effects:[{id:"minecraft:glowing"}]}] at @s run playsound block.note_block.chime player @s ~ ~ ~ 1 1 0
    execute if entity @s[nbt={active_effects:[{id:"minecraft:glowing"}]}] run effect clear @s glowing
