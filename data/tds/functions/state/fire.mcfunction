...
#ダメージ本体
    execute if score @s tds_fire matches 10.. run data merge storage tds: {Damage:1.0f,DamageType:2,DeathMessage:3,WeaponName:"",EPF:-1,BypassArmor:-1,BypassResistance:false,DisableParticle:true}
    execute if score @s tds_fire matches 10.. run scoreboard players operation #temp tds_dmg = @s tds_fire
    execute if score @s tds_fire matches 10.. store result storage tds: Damage float 0.1 run scoreboard players get #temp tds_dmg
